...
//
// Disk layout is: superblock, journal, inodes, block in-use bitmap, data blocks.
//
...
iput(struct inode *ip)
{
  ...
  if(ip->ref == 1 && (ip->flags & I_VALID) && ip->nlink == 0){
    ...
    ip->flags |= I_BUSY;
    release(&icache.lock);
    cprintf("(( itruc is called!\n");
    itrunc(ip);
    end_trans();
    jfull_flush();
    cprintf("(( itruc-ed was called!\n");
    ip->type = 0;
    iupdate(ip);
    acquire(&icache.lock);
    ip->flags &= ~I_BUSY;
...
bmap(struct inode *ip, uint bn, int alloc)
{
  ...
  if(bn < NININDIRECT){
    ...
    int indir_loc = bn/(BSIZE/sizeof(uint));
    if ((addr = a[indir_loc]) == 0) {
      if (!alloc) {
	brelse(bp);
	return -1;
      }
      
      a[indir_loc] = addr = balloc(ip->dev);
    ...
    indir_loc = bn % (BSIZE/sizeof(uint));
    if((addr = a[indir_loc]) == 0){
      if(!alloc){
        brelse(bp);
        return -1;
...
itrunc(struct inode *ip)
{
  ...
  if(ip->addrs[NDIRECT + 1]){
    ...
    a = (uint*)bp->data;
    int num_blocks_ref = BSIZE/sizeof(uint);
    for(j = 0; j < num_blocks_ref; j++) {
      if (a[j]) {
	ibp = bread(ip->dev, a[j]);
	b = (uint*)ibp->data;
	for (k = 0; k < num_blocks_ref; k++) {
	  bfree(ibp->dev, b[k]);
	}
	bfree(ip->dev, a[j]);
	brelse(ibp);
